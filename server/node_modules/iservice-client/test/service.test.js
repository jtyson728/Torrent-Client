...
describe('service test', function () {
  ...
  it('should_get_works_fine', function (done) {
    var obj = Service.create().subscribe('test');
    obj.start = function(){};
    obj.availables = [{
      'host' : 'host1',
      'port' : 'port1'
  ...
  it('should_set_works_fine', function (done) {
    ...
    var obj = Service.create(null, _store).subscribe('test');
    obj.start = function(){};
  ...
  /*{{{ should_noHB_works_fine */
  it('should_noHB_works_fine', function (done) {
    var server = http.createServer(function (req, res) {
      if (req.url === '/hbaddress') {
        res.end('ok');
      }
      ...
      setTimeout(function(){
        obj.availables.length.should.eql(2);
        server.close();
        done();
      }, 3000);
    });
